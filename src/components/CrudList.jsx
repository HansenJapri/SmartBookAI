...
export default function CrudList({ title, hint, items = [], placeholder, onAdd, onRename, onDelete, dupMsg, card = true }) {
  ...
  const add = async (e) => {
    ...
    catch (e2) { flash(e2.code === '23505' ? dup : (e2.message || c.failAdd)) }
  }
    ...
    <>
      ...
      <form className="crud-add" onSubmit={add}>
        ...
        <input className="input" value={name} onChange={(e) => setName(e.target.value)}
          placeholder={placeholder} aria-label={title ? c.addAria.replace('{title}', title) : c.addGeneric} />
        <button className="btn btn-primary">{c.add}</button>
      </form>
      <div className="crud-list">
        {items.length === 0 && <p className="muted-sm">{c.empty}</p>}
        {items.map((it) => (
          <div className={`crud-item ${editId === it.id ? 'editing' : ''}`} key={it.id}>
            {editId === it.id ? (
              <>
                <input className="input crud-input" value={editName} autoFocus
                  aria-label={c.editNameAria.replace('{name}', it.name)}
                  onChange={(e) => setEditName(e.target.value)}
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); saveEdit() } if (e.key === 'Escape') cancelEdit() }} />
                <button type="button" className="icon-btn ok" title={c.save} aria-label={c.save} onClick={saveEdit}><Check size={16} /></button>
                <button type="button" className="icon-btn" title={c.cancel} aria-label={c.cancel} onClick={cancelEdit}><X size={16} /></button>
              </>
            ) : (
              <>
                <span className="crud-name">{it.name}</span>
                <button type="button" className="icon-btn" title={c.edit} aria-label={c.edit} onClick={() => startEdit(it)}><Pencil size={15} /></button>
                <button type="button" className="icon-btn danger" title={c.delete} aria-label={c.delete} onClick={() => remove(it)}><Trash2 size={15} /></button>
              </>
            )}
          </div>
        ))}
