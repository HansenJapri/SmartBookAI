import { useEffect, useMemo, useState } from 'react'
import { ScrollText, RefreshCw } from 'lucide-react'
import { fetchAuditLogs, fetchStaff } from '../lib/api'
import { supabase } from '../lib/supabase'
import { fmtDateTime, rupiah } from '../lib/format'

// Label ramah untuk tabel & aksi.
const TABLE_LABEL = {
  transactions: 'Transaksi',
  products: 'Produk/Stok',
  suppliers: 'Pemasok',
  purchase_orders: 'Purchase Order',
  stock_opnames: 'Stock Opname',
  categories: 'Kategori',
  employees: 'Karyawan',
  payrolls: 'Penggajian',
  tasks: 'Tugas Operasional',
  attendance_rules: 'Aturan Absensi',
  kpi_bonus_rules: 'Jenjang KPI',
}
const ACTION_LABEL = {
  INSERT: { label: 'Tambah', cls: 'badge-green' },
  UPDATE: { label: 'Ubah', cls: 'badge-amber' },
  DELETE: { label: 'Hapus', cls: 'badge-red' },
}

// Ringkasan mudah dibaca dari payload jsonb log.
function summarize(log) {
  const c = log.changed || {}
  if (log.action === 'UPDATE') {
    const parts = Object.entries(c).slice(0, 3).map(([k, v]) => {
      const [oldV, newV] = Array.isArray(v) ? v : [null, v]
      const show = (x) => (x === null || x === undefined ? '-' : typeof x === 'object' ? '(data)' : String(x))
      return `${k}: ${show(oldV)} → ${show(newV)}`
    })
    const extra = Object.keys(c).length > 3 ? ` (+${Object.keys(c).length - 3} kolom lain)` : ''
    return parts.join('; ') + extra || '(tanpa perubahan kolom)'
  }
  const name = c.description || c.name || c.po_number || c.opname_number || ''
  const amt = c.amount ? ` — ${rupiah(Number(c.amount))}` : ''
  return `${name}${amt}` || `id ${String(log.row_id || '').slice(0, 8)}`
}

// Audit Log (owner): rekam jejak permanen siapa-mengubah-apa-kapan.
// Baris ditulis trigger database — aplikasi tidak bisa memalsukan/menghapusnya.
export default function Audit() {
  const [logs, setLogs] = useState(null)
  const [staff, setStaff] = useState([])
  const [me, setMe] = useState(null)
  const [table, setTable] = useState('')
  const [action, setAction] = useState('')
  const [err, setErr] = useState('')

  const load = () => {
    setErr('')
    fetchAuditLogs({ table, action }).then(setLogs).catch((e) => { setLogs([]); setErr(e.message) })
  }
  useEffect(() => { load() }, [table, action]) // eslint-disable-line
  useEffect(() => {
    fetchStaff().then(setStaff).catch(() => {})
    supabase.auth.getUser().then(({ data }) => setMe(data?.user?.id || null))
  }, [])

  const actorLabel = useMemo(() => (id) => {
    if (!id) return 'sistem'
    if (id === me) return 'Anda'
    const s = staff.find((x) => x.member_id === id)
    return s?.email || 'staf'
  }, [me, staff])

  if (!logs) return <div style={{ padding: 40, textAlign: 'center' }}><div className="spinner" style={{ margin: '0 auto' }} /></div>

  return (
    <>
      {err && <div className="alert alert-err">{err}</div>}
      <div className="toolbar">
        <select className="input" style={{ maxWidth: 200 }} value={table} onChange={(e) => setTable(e.target.value)}>
          <option value="">Semua data</option>
          {Object.entries(TABLE_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
        <select className="input" style={{ maxWidth: 160 }} value={action} onChange={(e) => setAction(e.target.value)}>
          <option value="">Semua aksi</option>
          {Object.entries(ACTION_LABEL).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
        </select>
        <div style={{ flex: 1 }} />
        <button className="btn btn-ghost" onClick={load}>
          <RefreshCw size={14} style={{ verticalAlign: '-2px', marginRight: 6 }} />Muat ulang
        </button>
      </div>

      {logs.length === 0 ? (
        <div className="card"><div className="empty">
          <div className="ee"><ScrollText size={36} /></div>
          <h3>Belum ada catatan</h3>
          <p>Setiap penambahan, perubahan, dan penghapusan data penting terekam otomatis di sini — permanen.</p>
        </div></div>
      ) : (
        <div className="table-wrap">
          <table className="tbl">
            <thead><tr><th>Waktu</th><th>Aksi</th><th>Data</th><th>Ringkasan</th><th>Pelaku</th></tr></thead>
            <tbody>
              {logs.map((l) => {
                const a = ACTION_LABEL[l.action] || { label: l.action, cls: 'badge-indigo' }
                return (
                  <tr key={l.id}>
                    <td className="muted-sm" style={{ whiteSpace: 'nowrap' }}>{fmtDateTime(l.created_at)}</td>
                    <td><span className={`badge ${a.cls}`}>{a.label}</span></td>
                    <td style={{ whiteSpace: 'nowrap' }}>{TABLE_LABEL[l.table_name] || l.table_name}</td>
                    <td style={{ maxWidth: 420, overflow: 'hidden', textOverflow: 'ellipsis' }}>{summarize(l)}</td>
                    <td className="muted-sm" style={{ whiteSpace: 'nowrap' }}>{actorLabel(l.actor_id)}</td>
                  </tr>
                )
              })}
            </tbody>
          </table>
        </div>
      )}
      <p className="muted-sm mt">
        Menampilkan maks. 200 catatan terbaru. Log ini ditulis langsung oleh database dan tidak bisa
        diubah/dihapus dari aplikasi — bukti jejak untuk keamanan usaha Anda.
      </p>
    </>
  )
}
