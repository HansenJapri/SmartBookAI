// Kunci layar (PIN) sisi-klien untuk dashboard admin. Mengunci tampilan setelah
// 10 menit tidak aktif atau saat meninggalkan tab terlalu lama, lalu meminta PIN.
// Lapisan privasi tambahan (bukan pengganti sesi login). PIN disimpan sebagai
// hash di perangkat ini saja.

export const LOCK_TIMEOUT_MS = 10 * 60 * 1000 // 10 menit

const PIN = (uid) => `bukupintar_admin_pin_${uid}`
const ACT = (uid) => `bukupintar_admin_active_${uid}`
const LOCK = (uid) => `bukupintar_admin_locked_${uid}`

export async function hashPin(pin) {
  const data = new TextEncoder().encode('bpai-admin:' + String(pin))
